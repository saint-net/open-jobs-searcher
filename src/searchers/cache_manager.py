...
class CacheManager:
    ...
    async def search_with_cache(self, url: str, domain: str) -> Optional[list[Job]]:
        ...
        for career_url in career_urls:
            try:
                ...
                if jobs_data:
                    # Check for suspicious drop (had many jobs, now very few)
                    prev_active = await self._repository.get_active_job_count(site.id)
                    is_suspicious = prev_active > 5 and len(jobs_data) < prev_active * 0.5

                    if is_suspicious:
                        url_id = career_url.id
                        count, should_sync = await self._repository.mark_url_suspicious(url_id)
                        if not should_sync:
                            logger.warning(
                                f"Suspicious: {prev_active} -> {len(jobs_data)}. "
                                f"Skipping ({count}/3)"
                            )
                            continue
                        logger.info(f"Job drop confirmed after {count} occurrences")
                    ...
                    # No jobs found - might be page load failure or suspicious
                    prev_active = await self._repository.get_active_job_count(site.id)
                    if prev_active > 5:
                        # Had many jobs before, now 0 - suspicious
                        url_id = career_url.id
                        count, should_sync = await self._repository.mark_url_suspicious(url_id)
                        logger.warning(
                            f"Suspicious: {prev_active} jobs -> 0. Confirmation {count}/3"
                        )
                        if not should_sync:
                            continue  # Skip, don't sync empty result
                    else:
                        # Few or no jobs before - might be normal
        ...
        if working_url:
            ...
            if career_domain != source_domain and career_domain != f"www.{source_domain}":
                ...
                logger.debug(f"Filtering jobs from {career_domain} for {source_domain}")
    ...
    async def save_to_cache(
        ...
        - Если было >5 вакансий и теперь 0 — не sync сразу
        - Увеличиваем suspicious_count, sync только после 3 подтверждений
        ...
        try:
            ...
            # Save career URL (get or create)
            career_url = await self._repository.add_career_url(site.id, clean_url, platform)
            logger.debug(f"Cached career URL for {domain}: {clean_url}")

            # PROTECTION: Check for suspicious job drop before syncing
            prev_active_count = await self._repository.get_active_job_count(site.id)

            # Suspicious drop: had jobs before, now significantly fewer or none
            is_suspicious_drop = (
                prev_active_count > 5 and
                len(jobs) < prev_active_count * 0.5  # >50% drop
            )

            if is_suspicious_drop:
                # Don't sync immediately - might be page load failure
                count, should_sync = await self._repository.mark_url_suspicious(career_url.id)

                if not should_sync:
                    # Not enough confirmations - skip sync, keep previous state
                    logger.warning(
                        f"Suspicious job drop for {domain}: {prev_active_count} -> {len(jobs)}. "
                        f"Skipping sync (confirmation {count}/3)"
                    )
                    self.last_sync_result = None
                    return
                else:
                    # Confirmed after multiple occurrences - proceed with sync
                    logger.info(
                        f"Job drop confirmed for {domain} after {count} occurrences. "
                        f"Syncing: {prev_active_count} -> {len(jobs)}"
                    )
            ...
            # Reset suspicious count on successful sync with jobs
            if jobs and career_url:
                await self._repository.mark_url_success(career_url.id)
    ...
    async def _maybe_extract_company_info(self, site, domain: str) -> None:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(main_page_url, timeout=timeout) as resp:
                    if resp.status == 200:
